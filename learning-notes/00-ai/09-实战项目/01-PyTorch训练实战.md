...
`requires_grad=True` 是关键开关。打开后，PyTorch 会记录这个张量参与的所有运算，构成计算图，`backward()` 时沿图回传——这就是 [反向传播推导](../machine-learning/04-神经网络原理/02-反向传播推导.md) 讲的那套机制的自动化版本。
...
详见 [学习率调度与训练技巧](../machine-learning/05-训练工程/04-学习率调度与训练技巧.md)。
...
BatchNorm 的 running stats 机制详见 [归一化技术](../machine-learning/05-训练工程/03-归一化技术.md)。
...
为什么优化器状态必须存：Adam 这类优化器内部维护一阶、二阶动量估计。丢了它们相当于重新「热身」，恢复后前几步的更新会失准。详见 [梯度下降与优化器](../machine-learning/04-神经网络原理/03-梯度下降与优化器.md)。
...
- 这些代码背后的数学 → [反向传播推导](../machine-learning/04-神经网络原理/02-反向传播推导.md)
- AdamW 与其他优化器的区别 → [梯度下降与优化器](../machine-learning/04-神经网络原理/03-梯度下降与优化器.md)
- 为什么用交叉熵而非 MSE → [损失函数](../machine-learning/04-神经网络原理/05-损失函数.md)
- BatchNorm 的 running stats 机制 → [归一化技术](../machine-learning/05-训练工程/03-归一化技术.md)
- 梯度裁剪、梯度累积、混合精度 → [学习率调度与训练技巧](../machine-learning/05-训练工程/04-学习率调度与训练技巧.md)
- 早停与过拟合监控 → [过拟合与偏差方差](../machine-learning/05-训练工程/01-过拟合与偏差方差.md)
